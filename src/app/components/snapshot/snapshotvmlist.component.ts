...
import { SnapshotLIST } from 'src/app/models/snapshotlist.model';

@Component({
selector: 'app-snapshotvmlist',
templateUrl: './snapshotvmlist.component.html',
styleUrls: ['./snapshotvmlist.component.css']
})
export class SnapshotComponent implements OnInit {
		...
		myInterval = setInterval(() =>{ this.reloadComponent(); }, 300000);
		selectedSnapshot: SnapshotLIST | any;
		constructor(
		...
		async getVMs(): Promise<void> {
			...
			let vms = data.items;
			const activesw = await lastValueFrom(this.k8sService.getActiveSW());
                        this.actstb = (activesw.split('\n'));
			...
			for (let i = 0; i < vms.length; i++) {
				...
				currentVm.actstb = "stb"
                                for (let i = 0; i < this.actstb.length; i++){
                                     if (currentVm.name == this.actstb[i] ) {
					currentVm.actstb = "act"
					break;
					}
   				}
				...
				if (currentVm.name.includes("node1")){
					const dataSs = await lastValueFrom(this.k8sService.getSnapshots1(currentVm.name));
					currentVm.snapshot = dataSs.snapshot;
				} else if (currentVm.name.includes("node2")){
					const dataSs = await lastValueFrom(this.k8sService.getSnapshots2(currentVm.name));
					currentVm.snapshot = dataSs.snapshot;
				} else {
					currentVm.snapshot = [
					{
		...
		async vmOperations(vmOperation: string, vmNamespace: string, vmName: string): Promise<void> {
			...
			if(vmOperation == "start"){
				...
				const userConfirmed = confirm('정말로 VM을 중지하시겠습니까?');
				if (userConfirmed) {
				...
				const userConfirmed = confirm('정말로 VM을 일시정지 하시겠습니까?');
                                if (userConfirmed) {
		...
		hideComponent(divId: string): void {
			...
			this.myInterval = setInterval(() =>{ this.reloadComponent(); }, 300000);
		}
		...
		reloadComponent(): void {
			this.router.navigateByUrl('/refresh',{skipLocationChange:true}).then(()=>{
					this.router.navigate([`/snapshotvmlist`]);
					})
		}

		async createSnapshot(vmname: string): Promise<void> {
			//console.log(vmname);
                      clearInterval(this.myInterval);
                      const userConfirmed = confirm('정말로 스냅샷을 생성하시겠습니까?');
                      if (userConfirmed) {
                        const loadingModal = document.getElementById("loading-modal");
 			if (loadingModal) {
 			    loadingModal.style.display = "block"; // 로딩 창 표시
			 try {
                         if(vmname.includes("node1")){
                            
				const data = await lastValueFrom(this.k8sService.createSnapshot1(vmname));
				if ( data.result == "success"){alert ("스냅샷 생성에 성공하였습니다");
					this.reloadComponent();
				} 
				else {alert ("스냅샷 생성에 실패하였습니다");}

				//console.log(data);
		     	        
                              }
			else if(vmname.includes("node2")){
				const data = await lastValueFrom(this.k8sService.createSnapshot2(vmname));
				if ( data.result == "success"){alert ("스냅샷 생성에 성공하였습니다");
					this.reloadComponent();
				}
				else {alert ("스냅샷 생성에 실패하였습니다");}
		...
		async deleteSnapshot(snapshotname: string): Promise<void> {
			...
			clearInterval(this.myInterval);
				const userConfirmed = confirm('정말로 스냅샷을 삭제하시겠습니까?');
                                if (userConfirmed) {
                                if (snapshotname === null){alert ("스냅샷을 선택하여주세요.");}
				else {
                       		    const loadingModal = document.getElementById("loading-modal-delete");
                                    if (loadingModal) {
                                    loadingModal.style.display = "block"; // 로딩 창 표시
                                    try {				 
					if(snapshotname.includes("node1")){
						const data = await lastValueFrom(this.k8sService.deleteSnapshot1(snapshotname));
						if ( data.result == "success"){
							alert ("스냅샷 삭제에 성공하였습니다");
							this.reloadComponent();
						}
						else{
							alert ("스냅샷 삭제에 실패하였습니다");
					...
					else if(snapshotname.includes("node2")){
						const data = await lastValueFrom(this.k8sService.deleteSnapshot2(snapshotname));
						if ( data.result == "success"){alert ("스냅샷 삭제에 성공하였습니다");
							this.reloadComponent();
						}
						else{alert ("스냅샷 삭제에 실패하였습니다");}
					}
					else {
						alert ("스냅샷 삭제에 실패하였습니다");
					}
		...
		async restoreSnapshot(vmname: string, snapshotname: string): Promise<void> {
			clearInterval(this.myInterval);
			const userConfirmed = confirm('정말로 스냅샷을 복원하시겠습니까?');
                        if (userConfirmed) {
                        if (snapshotname === null){alert ("스냅샷을 선택하여주세요.");}
			else {
                              const loadingModal = document.getElementById("loading-modal-restore");
                              if (loadingModal) {
                              loadingModal.style.display = "block"; // 로딩 창 표시
                              try {
				if(snapshotname.includes("node1")){
					const data = await lastValueFrom(this.k8sService.restoreSnapshot1(vmname, snapshotname));
					if ( data.result == "success"){
						alert ("스냅샷 복원에 성공하였습니다");
						this.reloadComponent();
					}
					else  {
						alert ("스냅샷 복원에 실패하였습니다");
					}
					//console.log(data);
				}
				else if(snapshotname.includes("node2")){
					const data = await lastValueFrom(this.k8sService.restoreSnapshot2(vmname, snapshotname));
					if ( data.result == "success"){alert ("스냅샷 복원에 성공하였습니다");
						this.reloadComponent();
					}
					else  {alert ("스냅샷 복원에 실패하였습니다");}
				}
				else {
					alert ("스냅샷 복원에 실패하였습니다");
				}
                        ...
                        console.error("스냅샷 복원 중 오류 발생:", error);
                        alert("스냅샷 복원 중 오류가 발생했습니다");
                    } finally {
                        if (loadingModal) {
                           loadingModal.style.display = "none";
                        }	
		...
		// Searchsnapshot(vmname: string, selectedSnapshot: string): Promise<SnapshotLIST | null> {
		//for (let i = 0; i < this.vmList.length; i++) {
		//	 if (this.vmList[i].name === vmname
		//		 && this.vmList[i].snapshot == selectedSnapshot) {
		...
		// }
		async Searchsnapshot(vmname: string, selectedSnapshot: string): Promise<SnapshotLIST | null> {
			for (let i = 0; i < this.vmList.length; i++) {
				if (this.vmList[i].name === vmname) {
					const snapshots = this.vmList[i].snapshot;
					for (let j = 0; j < snapshots.length; j++) {
						if (snapshots[j].name === selectedSnapshot) {
							return snapshots[j]; // 선택한 스냅샷 정보를 반환합니다.
						}
					}
				}
			}
			return null; // 해당 VM 또는 스냅샷을 찾지 못한 경우 null을 반환합니다.
		}
		isSnapshotSelected(thisVM: KubeVirtVM, selectedSnapshot: SnapshotLIST): boolean {
			if (!selectedSnapshot){return false;}
			//console.log(selectedSnapshot);
			// console.log(thisVM);

			return thisVM.snapshot.some(snapshot => snapshot.name === selectedSnapshot.name);
